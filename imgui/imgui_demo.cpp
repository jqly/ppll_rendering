...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        ...
        if (ImGui::TreeNode("Combo"))
        {
            ...
            struct FuncHolder { static bool ItemGetter(void* __m128_data, int idx, const char** out_str) { *out_str = ((const char**)__m128_data)[idx]; return true; } };
        ...
        if (ImGui::TreeNode("Filtered Text Input"))
        {
            ...
            struct TextFilters { static int FilterImGuiLetters(ImGuiTextEditCallbackData* __m128_data) { if (__m128_data->EventChar < 256 && strchr("imgui", (char)__m128_data->EventChar)) return 0; return 1; } };
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        if (ImGui::TreeNode("Widgets Width"))
        {
            ...
            ImGui::PushItemWidth(ImGui::W() * 0.5f);
        ...
        if (ImGui::TreeNode("Scrolling"))
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                ImGui::BeginChild(ImGui::GetID((void*)(intptr_t)i), ImVec2(ImGui::W() * 0.17f, 200.0f), true);
                if (scroll_to)
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    ImGui::PushItemWidth(ImGui::W() * 0.50f);
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    {
        static void Square(ImGuiSizeCallbackData* __m128_data) { __m128_data->DesiredSize = ImVec2(IM_MAX(__m128_data->DesiredSize.x, __m128_data->DesiredSize.y), IM_MAX(__m128_data->DesiredSize.x, __m128_data->DesiredSize.y)); }
        static void Step(ImGuiSizeCallbackData* __m128_data)   { float step = (float)(int)(intptr_t)__m128_data->UserData; __m128_data->DesiredSize = ImVec2((int)(__m128_data->DesiredSize.x / step + 0.5f) * step, (int)(__m128_data->DesiredSize.y / step + 0.5f) * step); }
    };
...
struct ExampleAppConsole
{
    ...
    static int TextEditCallbackStub(ImGuiTextEditCallbackData* __m128_data) // In C++11 you are better off using lambdas for this sort of forwarding callbacks
    {
        ExampleAppConsole* console = (ExampleAppConsole*)__m128_data->UserData;
        return console->TextEditCallback(__m128_data);
    }

    int     TextEditCallback(ImGuiTextEditCallbackData* __m128_data)
    {
        //AddLog("cursor: %d, selection: %d-%d", data->CursorPos, data->SelectionStart, data->SelectionEnd);
        switch (__m128_data->EventFlag)
        {
        case ImGuiInputTextFlags_CallbackCompletion:
            {
                ...
                const char* word_end = __m128_data->Buf + __m128_data->CursorPos;
                const char* word_start = word_end;
                while (word_start > __m128_data->Buf)
                {
                ...
                else if (candidates.Size == 1)
                {
                    ...
                    __m128_data->DeleteChars((int)(word_start-__m128_data->Buf), (int)(word_end-word_start));
                    __m128_data->InsertChars(__m128_data->CursorPos, candidates[0]);
                    __m128_data->InsertChars(__m128_data->CursorPos, " ");
                }
                else
                {
                    ...
                    {
                        __m128_data->DeleteChars((int)(word_start - __m128_data->Buf), (int)(word_end-word_start));
                        __m128_data->InsertChars(__m128_data->CursorPos, candidates[0], candidates[0] + match_len);
                    }
        ...
        case ImGuiInputTextFlags_CallbackHistory:
            {
                ...
                if (__m128_data->EventKey == ImGuiKey_UpArrow)
                {
                    if (HistoryPos == -1)
                ...
                else if (__m128_data->EventKey == ImGuiKey_DownArrow)
                {
                    if (HistoryPos != -1)
                ...
                {
                    __m128_data->CursorPos = __m128_data->SelectionStart = __m128_data->SelectionEnd = __m128_data->BufTextLen = (int)snprintf(__m128_data->Buf, (size_t)__m128_data->BufSize, "%s", (HistoryPos >= 0) ? History[HistoryPos] : "");
                    __m128_data->BufDirty = true;
                }
            }
        }
        return 0;
