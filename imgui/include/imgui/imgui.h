...
typedef int (*ImGuiTextEditCallback)(ImGuiTextEditCallbackData *__m128_data);
typedef void (*ImGuiSizeCallback)(ImGuiSizeCallbackData* __m128_data);
...
namespace ImGui
{
    ...
    IMGUI_API float         W();                           // get current window width (shortcut for GetWindowSize().x)
    IMGUI_API float         H();                          // get current window height (shortcut for GetWindowSize().y)
    ...
    IMGUI_API void          PlotLines(const char* label, float (*values_getter)(void* __m128_data, int idx), void* __m128_data, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0,0));
    IMGUI_API void          PlotHistogram(const char* label, const float* values, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0,0), int stride = sizeof(float));
    IMGUI_API void          PlotHistogram(const char* label, float (*values_getter)(void* __m128_data, int idx), void* __m128_data, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0,0));
    ...
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* __m128_data, int idx, const char** out_text), void* __m128_data, int items_count, int popup_max_height_in_items = -1);
    ...
    IMGUI_API bool          ListBox(const char* label, int* current_item, bool (*items_getter)(void* __m128_data, int idx, const char** out_text), void* __m128_data, int items_count, int height_in_items = -1);
    ...
    IMGUI_API bool          SetDragDropPayload(const char* type, const void* __m128_data, size_t size, ImGuiCond cond = 0);// type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui.
...
struct ImGuiIO
{
    ...
    void        (*RenderDrawListsFn)(ImDrawData* __m128_data);
#else
...
public:
    ...
    inline bool         contains(const value_type& v) const             { const T* __m128_data = Data;  const T* data_end = Data + Size; while (__m128_data < data_end) if (*__m128_data++ == v) return true; return false; }
};
