...
ImU32 ImHash(const void* __m128_data, int data_size, ImU32 seed)
{
    ...
    seed = ~seed;
    ImU32 crc = seed;
    const unsigned char* current = (const unsigned char*)__m128_data;
...
static ImVector<ImGuiStorage::Pair>::iterator LowerBound(ImVector<ImGuiStorage::Pair>& __m128_data, ImGuiID key)
{
    ImVector<ImGuiStorage::Pair>::iterator first = __m128_data.begin();
    ImVector<ImGuiStorage::Pair>::iterator last = __m128_data.end();
...
static ImVec2 CalcSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
    ...
    if (g.NextWindowData.SizeConstraintCond != 0)
    {
        ...
        {
            ImGuiSizeCallbackData __m128_data;
            __m128_data.UserData = g.NextWindowData.SizeCallbackUserData;
            __m128_data.Pos = window->Pos;
            __m128_data.CurrentSize = window->SizeFull;
            __m128_data.DesiredSize = new_size;
            g.NextWindowData.SizeCallback(&__m128_data);
            new_size = __m128_data.DesiredSize;
        }
    }
...
void ImGui::PopStyleVar(int count)
{
    ...
    while (count > 0)
    {
        ...
        void* __m128_data = info->GetVarPtr(&g.Style);
        if (info->Type == ImGuiDataType_Float && info->Count == 1)      { ((float*)__m128_data)[0] = backup.BackupFloat[0]; }
        else if (info->Type == ImGuiDataType_Float && info->Count == 2) { ((float*)__m128_data)[0] = backup.BackupFloat[0]; ((float*)__m128_data)[1] = backup.BackupFloat[1]; }
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* __m128_data, int idx), void* __m128_data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    if (scale_min == FLT_MAX || scale_max == FLT_MAX)
    {
        ...
        {
            const float v = values_getter(__m128_data, i);
            v_min = ImMin(v_min, v);
            v_max = ImMax(v_max, v);
        }
    ...
    if (values_count > 0)
    {
        ...
        if (hovered)
        {
            ...
            const float v0 = values_getter(__m128_data, (v_idx + values_offset) % values_count);
            const float v1 = values_getter(__m128_data, (v_idx + 1 + values_offset) % values_count);
        ...
        float v0 = values_getter(__m128_data, (0 + values_offset) % values_count);
        float t0 = 0.0f;
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            const float v1 = values_getter(__m128_data, (v1_idx + values_offset + 1) % values_count);
...
static float Plot_ArrayGetter(void* __m128_data, int idx)
{
    ImGuiPlotArrayGetterData* plot_data = (ImGuiPlotArrayGetterData*)__m128_data;
...
{
    ImGuiPlotArrayGetterData __m128_data(values, stride);
    PlotEx(ImGuiPlotType_Lines, label, &Plot_ArrayGetter, (void*)&__m128_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}

void ImGui::PlotLines(const char* label, float (*values_getter)(void* __m128_data, int idx), void* __m128_data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    PlotEx(ImGuiPlotType_Lines, label, values_getter, __m128_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}

void ImGui::PlotHistogram(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride)
{
    ImGuiPlotArrayGetterData __m128_data(values, stride);
    PlotEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&__m128_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}

void ImGui::PlotHistogram(const char* label, float (*values_getter)(void* __m128_data, int idx), void* __m128_data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    PlotEx(ImGuiPlotType_Histogram, label, values_getter, __m128_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* __m128_data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;

    const char* preview_text = NULL;
    if (*current_item >= 0 && *current_item < items_count)
        items_getter(__m128_data, *current_item, &preview_text);
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (!items_getter(__m128_data, i, &item_text))
...
static bool Items_ArrayGetter(void* __m128_data, int idx, const char** out_text)
{
    const char* const* items = (const char* const*)__m128_data;
    if (out_text)
        *out_text = items[idx];
    return true;
}

static bool Items_SingleStringGetter(void* __m128_data, int idx, const char** out_text)
{
    // FIXME-OPT: we could pre-compute the indices to fasten this. But only 1 active combo means the waste is limited.
    const char* items_separated_by_zeros = (const char*)__m128_data;
    int items_count = 0;
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* __m128_data, int items_count, int height_in_items)
{
...
bool ImGui::SetDragDropPayload(const char* type, const void* __m128_data, size_t data_size, ImGuiCond cond)
{
    ImGuiContext& g = *GImGui;
    ImGuiPayload& payload = g.DragDropPayload;
    if (cond == 0)
    ...
    IM_ASSERT((__m128_data != NULL && data_size > 0) || (__m128_data == NULL && data_size == 0));
    ...
    if (cond == ImGuiCond_Always || payload.DataFrameCount == -1)
    {
        ...
        if (data_size > sizeof(g.DragDropPayloadBufLocal))
        {
            ...
            memcpy(payload.Data, __m128_data, data_size);
        }
        else if (data_size > 0)
        {
            // Store locally
